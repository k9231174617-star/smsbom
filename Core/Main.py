...
def main(page: Page):
    ...
    def color_change(e):
        ...
        banner.controls = [
            Text(
                spans=[TextSpan('Grobovsheke', TextStyle(size=85, foreground=Paint(color=color, stroke_width=9, stroke_join='round', style='stroke')))],
                font_family='Consolas'
            ),
            Text(
                spans=[TextSpan('GROBOVSHEKE', TextStyle(size=95, color=color))],
                font_family='Consolas'
            )
        ]
    ...
    def error(message, reason='error'):
        ...
        error_message = AlertDialog(
            ...
            actions=[TextButton('ОКЕЙ', on_click=button_cancel, style=ButtonStyle(color=color))],
    ...
    def start_attack():
        ...
        attack_window = AlertDialog(
            modal=True,
            title=Text('attack started', color=color, size=30, text_align=TextAlign.CENTER),
            content=ProgressBar(width=325, color=color),
            actions=[TextButton('close', width=90, height=40, on_click=button_cancel, style=ButtonStyle(color=color))],
            actions_alignment='end',
            open=True
        )
    ...
    def confirmation():
        ...
        confirmation_window = AlertDialog(
            modal=True,
            title=Text('Attention!', color=color, size=30, text_align='center'),
            content=Text('After launching an attack and instantly canceling it, the launch process will be irreversible, and the attack will be completed to completion in any case!\n\nContinue?'),
            actions=[
                TextButton('NO', on_click=button_cancel, style=ButtonStyle(color=color)),
                TextButton('YES', on_click=button_continue, style=ButtonStyle(color=color))
            ],
            actions_alignment='end',
            open=True
        )
    ...
    def information(e):
        ...
        information_window = AlertDialog(
            content=Text('Grobovsheke-SmsBomber!', text_align='center', size=24, color=color, font_family='Consolas'),
            open=True,
    ...
    banner = Stack([
        Text(
            spans=[TextSpan('Grobovsheke', TextStyle(size=85, foreground=Paint(color=color, stroke_width=7, stroke_join='round', style='stroke')))],
            font_family='Consolas'
        ),
        Text(
            spans=[TextSpan('Grobovsheke', TextStyle(size=85, color=color))],
            font_family='Consolas'
        )
    ])

    number = TextField(
        label='Enter the number without "+"',
        width=275,
        text_align='center',
        border_radius=40,
        border_color=color,
    ...
    feedback = Switch(
        ...
        value=True if check_config()['feedback'] == 'True' else False,
        width=280,
        active_color=color,
        on_change=feedback_change,
        tooltip='Services that leave applications (such as connecting to the Internet or taking out a loan) on different sites. Be careful when using this function!'
    )

    attack_button = ElevatedButton(
        content=Text('Attack', size=25),
    ...
    def ADD():
        page.add(
            ...
            attack_button,
            Row([
                IconButton(icon='telegram', icon_size=48, tooltip='Channel', url='https://t.me/grobovsheke', icon_color=color),
                ...
                IconButton(icon='info', icon_size=48, tooltip='Information', icon_color=color, on_click=information)
            ], alignment='CENTER')
        )

    ADD()

def Start(web=True, port=9876):
    if web:
        host = '0.0.0.0'  # Railway требует слушать на 0.0.0.0
